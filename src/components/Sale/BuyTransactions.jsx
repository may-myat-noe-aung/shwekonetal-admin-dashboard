...
const BuyTransactions = ({ sales, updateStatus }) => {
    ...
    if (sortConfig.key) {
      sortable.sort((a, b) => {
        ...
        if (sortConfig.key === "time") {
          aVal = a.date.getHours() * 3600 + a.date.getMinutes() * 60 + a.date.getSeconds();
          bVal = b.date.getHours() * 3600 + b.date.getMinutes() * 60 + b.date.getSeconds();
        } else {
  ...
  const exportCSV = () => {
    ...
    link.download = "buy_transactions.csv";
    link.click();
  };
 ...
 <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-4 mb-6 overflow-x-auto">
  ...
  <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-3 gap-3">
    <h3 className="text-xl font-semibold text-green-400">Buy Transactions</h3>
        ...
        {paginatedSales.length === 0 ? (
          ...
          paginatedSales.map((s) => (
            ...
            >
              ...
              {/* <td className="py-1 px-3 capitalize font-semibold text-green-400 whitespace-nowrap">{s.type}</td> */}
              <td className="py-2 px-3 whitespace-nowrap">{s.gold}</td>
              <td className="py-2 px-3 whitespace-nowrap">{s.price.toLocaleString()} Ks</td>
              <td className="py-2 px-3 whitespace-nowrap">{s.date.toLocaleDateString()}</td>
              <td className="py-2 px-3 whitespace-nowrap">{s.date.toLocaleTimeString()}</td>
              <td className="py-2 px-3 capitalize whitespace-nowrap">{s.method}</td>
              <td
