...
export default function UserTableWithSummary() {
  ...
  const today = new Date().toISOString().slice(0, 10);
  const newUsersCount = users.filter(
    (u) => u.create_at && u.create_at.slice(0, 10) === today
  ).length;
  ...
  const filteredUsers = useMemo(() => {
    return users
      .filter((u) => u.status !== "pending") // ✅ exclude pending
      .filter((u) => {
        const matchesSearch = [u.fullname, u.email, u.id_type, u.id_number]
          .join(" ")
          .toLowerCase()
          .includes(searchQuery.toLowerCase());
        const matchesRole =
          roleFilter === "All" ? true : u.promoter === roleFilter;
        return matchesSearch && matchesRole;
      })
      .filter((u) => {
    ...
    <main className="bg-neutral-950 text-neutral-100 ">
      <div className="mx-auto max-w-7xl px-4 py-6 space-y-6">
        ...
        <SummaryCards
          ...
          approved={approvedCount}
          rejected={rejectedCount}
          newUsers={newUsersCount} // ✅ now works
        />
  ...
  {u.create_at
    ? new Date(u.create_at).toLocaleDateString("en-GB")
    : "—"}
</td>
