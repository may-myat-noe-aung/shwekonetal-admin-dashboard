...
const API_BASE = "http://38.60.244.74:3000"; // backend URL
...
export default function BuyingPricesChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (keys.length < 2) return; // data not enough

        const todayDate = keys[0];      // first key = Today
        const yesterdayDate = keys[1];  // second key = Yesterday

        // --- Step 2: Convert to array & filter null values ---
        const convertToArray = (dateStr) => {
          if (!data[dateStr]) return [];
          return Object.entries(data[dateStr])
            .filter(([_, price]) => price !== null) // skip nulls
        ...
        // --- Step 3: Merge time points for chart ---
  ...
  <AreaChart data={chartData}>
    ...
    <Tooltip
      formatter={(value, name) => [`${value} ကျပ်`, name]} // <-- added
      contentStyle={{
        background: "#0a0a0a",
